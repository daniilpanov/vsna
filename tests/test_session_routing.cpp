...
// A syntactically valid hello used to complete the mandatory hello handshake
// before a routing frame is sent.
std::string validHello()
{
	return json{ { "type", "hello" }, { "payload", json::object() } }.dump();
}
  ...
  public:
	...
	void start()
	{
		_thread = std::thread([this] {
			...
			ws.text(true);
			ws.write(asio::buffer(validHello()));

			// The dialed NodeSession greets with a hello frame on handshake.
			// Consume it, then send our routing frame and read its reply.
...
class RawClientSendsThenReads {
  public:
	...
	std::string run()
	{
		...
		// The accepting NodeSession greets with a hello frame on connect.
		// Consume it, complete the mandatory hello handshake, then send our
		// routing frame and read the reply.
		beast::flat_buffer greeting;
		ws.read(greeting);
...
class RawClientSendsTwoFramesThenReads {
  public:
	...
	std::string run()
	{
		...
		// The accepting NodeSession greets with a hello frame on connect.
		// Consume it, complete the mandatory hello handshake, then send the
		// frames under test.
		beast::flat_buffer greeting;
		ws.read(greeting);
...
// A raw WebSocket client that sends three frames then reads one reply.
// Used to test the pre-init ping path: a ping sent before the handshake-completing
// hello must be swallowed, then the hello completes init and the third frame routes.
class RawClientSendsThreeFramesThenReads {
  public:
	explicit RawClientSendsThreeFramesThenReads(uint16_t port, std::string first,
	                                            std::string second, std::string third)
	    : _port(port), _first(std::move(first)), _second(std::move(second)),
	      _third(std::move(third))
	{}
	...
	std::string run()
	{
		...
		// The accepting NodeSession greets with a hello frame on connect. Consume
		// it; note we do NOT send a validating hello right away — the frames under
		// test begin with a ping that exercises the pre-init branch.
		beast::flat_buffer greeting;
		ws.read(greeting);

		ws.write(asio::buffer(_first));
		ws.write(asio::buffer(_second));
		ws.write(asio::buffer(_third));
...
// A raw WebSocket server that reads two frames from the dialed peer then replies
// with one frame. Used to test the dial direction: the server sends frames (e.g. a
// ping then a claim) and reads the session's reply.
  ...
  public:
	...
	void start()
	{
		_thread = std::thread([this] {
			...
			// Reply with our own hello to complete the handshake.
			ws.write(asio::buffer(validHello()));

			// Send the frames under test, then read the session's reply.
			ws.write(asio::buffer(_first));
			ws.write(asio::buffer(_second));
...
// A raw WebSocket server that reads three frames then replies with one. Used to
// test the dial-direction pre-init path: the server sends a ping (before replying
// with hello), then after the handshake a claim, and reads the session's reply.
class RawServerSendsThreeFramesThenReads {
  public:
	RawServerSendsThreeFramesThenReads(uint16_t port, std::string first, std::string second,
	                                   std::string third)
	    : _port(port), _first(std::move(first)), _second(std::move(second)),
	      _third(std::move(third))
	{}
	...
	void start()
	{
		_thread = std::thread([this] {
			...
			// Send the first frame (a ping) before replying with hello — this
			// exercises the pre-init branch in the dial direction.
			ws.write(asio::buffer(_first));

			// Complete the handshake, then send the claim and read the reply.
			ws.write(asio::buffer(_second));
			ws.write(asio::buffer(_third));
...
TEST(PersistentSessionDial, RoutesMessageAndReplies)
{
	...
	RawServerSendsThenReads server(
	    port, json{ { "type", "claim" }, { "payload", json::object() } }.dump());
	server.start();
...
// Accept direction: after the hello handshake a raw client sends a garbage
// frame followed by a valid JSON frame. The session ignores the garbage and
// routes the valid message. Proves that post-handshake on_read survives non-JSON
// input without breaking the connection.
TEST(PersistentSessionAccept, InvalidJSONSessionStaysAlive)
{
	...
	std::string valid = json{ { "type", "status" }, { "payload", json::object() } }.dump();
...
TEST(PersistentSessionAccept, EmptyPayloadRoutedCorrectly)
{
	...
	std::string frame = json{ { "type", "claim" } }.dump();
...
// Accept direction: a ping sent before a normal data frame does not disrupt the
// flow — the ping is dropped and the following message still routes and replies.
...
// A ping frame sent BEFORE the post-connect hello handshake completes must be
// swallowed by the pre-init branch of on_read (it must not tear the connection
// down). The handshake then still completes on the following valid hello, and a
// subsequent claim still routes and replies.
TEST(PersistentSession, IncomingPingBeforeHelloHandshakeSurvives)
{
	auto port = freePort();
	asio::io_context ioc;
	Node node;
	...
	RawClientSendsThreeFramesThenReads client(port, std::move(ping), std::move(hello),
	                                          std::move(claim));
	std::string replyText;
	std::string clientErr;
	std::thread clientThread([&] {
	...
	ASSERT_TRUE(ready(got)) << "claim handler did not fire after a pre-init ping";
	clientThread.join();
	EXPECT_TRUE(clientErr.empty()) << "client error: " << clientErr;
	json j2 = json::parse(replyText);
	EXPECT_EQ(j2.at("type").get<std::string>(), "status");

	ioc.stop();
	worker.join();
}

// Dial direction, post-handshake: the dialed NodeSession is already initialized
// when a raw server sends a ping followed by a claim. The ping is swallowed by the
// post-init branch and the claim still routes and replies.
TEST(PersistentSessionDial, IncomingPingIgnored)
{
	...
	std::string reply = server.result();
	json j3 = json::parse(reply);
	EXPECT_EQ(j3.at("type").get<std::string>(), "status");
...
// Dial direction, pre-handshake: a ping arrives from the raw server BEFORE the
// session's hello handshake is completed by a valid hello reply. The ping is
// swallowed by the pre-init branch, the handshake completes on the hello, and a
// subsequent claim still routes and replies.
TEST(PersistentSessionDial, IncomingPingBeforeHelloIgnored)
{
	auto port = freePort();
	RawServerSendsThreeFramesThenReads server(
	    port, json{ { "type", "ping" }, { "payload", json::object() } }.dump(), validHello(),
	    json{ { "type", "claim" }, { "payload", json::object() } }.dump());
	server.start();
	ASSERT_TRUE(server.waitBound()) << "server did not start listening";
	...
	ASSERT_TRUE(ready(got)) << "claim handler did not fire after a pre-init dial-direction ping";

	std::string reply = server.result();
	json j4 = json::parse(reply);
	EXPECT_EQ(j4.at("type").get<std::string>(), "status");
