...
void PeerRegistry::addConnected(const std::string& addr, std::shared_ptr<NodeSession> session)
{
	std::lock_guard<std::mutex> lock(_mutex);
	_known.insert(addr);
	auto& slots = _connected[addr];
	for (const auto& s : slots)
	{
		if (s.get() == session.get())
			return;
	}
	slots.push_back(std::move(session));
}

void PeerRegistry::markConnected(const std::string& addr)
{
	std::lock_guard<std::mutex> lock(_mutex);
	_known.insert(addr);
	auto& slots = _connected[addr];
...
void PeerRegistry::disconnect(const std::string& addr, NodeSession *session)
{
	std::lock_guard<std::mutex> lock(_mutex);
